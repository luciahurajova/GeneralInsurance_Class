...
```{r}
dt_KPI_raw[is.na(dt_KPI_raw$Business), ]
...
##### Which Unit has collected the most Premium? #Unit7
...
##### Which Unit has the lowest Expenses? #Unit 5
```{r}
dt_KPI_raw %>% 
  group_by(Unit) %>% 
...
##### Which Business from Unit found in the previous task had the highest Expenses? #Travel
```{r}
dt_KPI_raw %>% filter(Unit=="Unit5") %>% arrange(desc(Expenses)) 
```
...
dt_KPI_raw%>%
  ...
  summarize(Premium = sum(Premium, na.rm = TRUE),
            Expenses = sum(Expenses, na.rm = TRUE),
            Losses = sum(Losses, na.rm = TRUE))%>% 
  mutate(res=Premium-Expenses-Losses) %>%
  arrange(desc(res))
```
...
##### Which Unit looks bad in terms of Losses? #Unit8 
```{r}
dt_KPI_raw %>% 
  group_by(Unit) %>% 
  summarize(Premium = sum(Premium, na.rm = TRUE),
  ...
  ggplot(aes(x = reorder(Unit, -Losses), y = Losses)) + 
  geom_col()
```

##### Which Unit looks pretty good in terms of Expenses? #Unit5
```{r}
dt_KPI_raw %>% 
  group_by(Unit) %>% 
  summarize(Premium = sum(Premium, na.rm = TRUE),
...
##### Which Business from previous Unit looks bad in terms of Expenses? #travel
```{r}
dt_KPI_raw %>% 
  filter(Unit=="Unit5") %>% 
  ggplot(aes(x = reorder(Business, -Expenses), y = Expenses)) + 
...
```{r}
dt_KPI_raw %>% mutate(cost=Expenses+Losses)%>%
  group_by(Unit) %>% 
summarize(cost = sum(cost, na.rm = TRUE),
          Premium = sum(Premium, na.rm = TRUE))%>%
mutate(Loss_ratio=cost/Premium)
  

```


##### Which Unit looks pretty good in terms of UWR? #Unit7
```{r}
...
##### Which Business of previous Unit looks bad in terms of UWR? #Housing
```{r}
dt_KPI_raw %>% mutate(uwr=Premium-Expenses-Losses)%>%
  filter(Unit=="Unit5") %>% 
  arrange(uwr)
